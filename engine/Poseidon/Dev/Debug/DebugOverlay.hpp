...
// touching the engine.
//
// Lifecycle (called by the active graphics backend + main loop):
//   DebugOverlay::Init(window, renderer) — once after the graphics device exists
//   DebugOverlay::ProcessEvent(event)    — for every SDL event
//   DebugOverlay::NewFrame()             — at the start of each overlay frame
//   DebugOverlay::Render()               — at the backend's overlay-composite point
...
{
/// Takes ownership of `renderer`. Passing nullptr leaves the overlay
/// uninitialized and every entry point remains a safe no-op.
