...
// Detect a Texture whose backend surface was freed during hot reload. The
// CPU-side atlas pixels remain valid, but UpdateDynamic must not target a
// dead GL name or stale Metal registry handle.
static void SyncAtlasTextures(Engine* engine, Poseidon::ui::FontRenderer* fr)
{
    ...
    for (int i = 0; i < pageCount; i++)
    {
        ...
        if (textures[i] && !textures[i]->IsGpuResident())
            textures[i] = nullptr;
